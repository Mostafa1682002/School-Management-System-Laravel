...
@section('title')
    {{ trans('Subject_trans.edite_page_title') }}
@endsection
@section('css')
@endsection
@section('page-header')
    ...
    <div class="page-title">
        <div class="row">
            <div class="col-sm-6">
                <h4 class="mb-0">{{ trans('Subject_trans.edite_page_title') }}</h4>
            </div>
            <div class="col-sm-6">
                <ol class="breadcrumb pt-0 pr-0 float-left float-sm-right ">
                    <li class="breadcrumb-item"><a href="#" class="default-color">{{ trans('main_trans.Subjects') }}</a>
                    </li>
                    <li class="breadcrumb-item active">{{ trans('Subject_trans.edite_page_title') }}</li>
                </ol>
            </div>
        </div>
    </div>
                        ...
                        <div class="form-row">
                            <div class="form-group col">
                                ...
                                <select class="custom-select my-1 mr-sm-2" name="school_grade_id">
                                    <option value="{{ $subject->school_grade_id }}">{{ $subject->grade->name }}
                                    </option>
                            ...
                            <div class="form-group col">
                                ...
                                <select class="custom-select my-1 mr-sm-2" name="classe_id">
                                    <option value="{{ $subject->classe_id }}">{{ $subject->classe->class_name }}</option>
                                </select>
                                @error('classe_id')
                                    <div class="alert alert-danger">{{ $message }}</div>
                                @enderror
                        ...
                        <div class="form-row">
                            <div class="form-group col">
                                ...
                                <select class="custom-select my-1 mr-sm-2" name="teacher_id">
                                    <option value="{{ $subject->teacher_id }}">{{ $subject->teacher->name_teacher }}
                                    </option>
